...
BoussinesqKE.sh . 0 $startAverageTime 'stable up down'
...
horizontalMean -case . -time $startAverageTime':' hMeans 10 \
    '(sigma.stable sigma.up sigma.down
      ...
      sigmauSqr.stable sigmauSqr.up sigmauSqr.down
      heatTransferz P KE)'
...
for time in [0-9]*; do
    for var in p b u uSqr; do for part in stable up down; do
        if [ $time -ge $startAverageTime ]; then
...
# Time averages
for part in stable up down; do for var in sigma p b u uSqr; do
