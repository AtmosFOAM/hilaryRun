...
time=$1
alpha=`grep alpha system/environmentalProperties | awk 'NR==1{print $8}' |\
    awk -F';' '{print $1}'`
nu=`grep nu system/environmentalProperties | awk 'NR==1{print $8}' |\
    awk -F';' '{print $1}'`
...
for var in b u TKE nuTurb P sigma massTransfer.stable massTransfer.up massTransfer.down; do
    for part in '' .stable .up .down; do
...
sed 's/TIME/'$time'/g' gmtFiles/massTransfer.gmt > gmtFiles/tmp.gmt; \
    gmtPlot gmtFiles/tmp.gmt
sed 's/TIME/'$time'/g' gmtFiles/TKE.gmt > gmtFiles/tmp.gmt; \
    gmtPlot gmtFiles/tmp.gmt
sed 's/TIME/'$time'/g' gmtFiles/nuTurb.gmt > gmtFiles/tmp.gmt; \
    gmtPlot gmtFiles/tmp.gmt
rm gmtFiles/tmp.gmt
