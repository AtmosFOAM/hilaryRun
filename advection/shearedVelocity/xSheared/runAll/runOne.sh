...
dt=`echo $cValue $res | awk '{print 2*$1/$2}'`
...
if [[ ! -f $resDir/constant/polyMesh/points ]]; then
    ...
    sed -i 's/NX/'$twoRes'/g' $resDir/system/blockMeshDict
...
if [[ ! -f $resDir/analytic/0.25/T_analytic ]]; then
    ...
    gmtFoam -case $resDir/analytic T
...
if [[ $MPDATAtype == exp ]]; then
    ...
    echo MPDATAtype $MPDATAtype not recognised. Should be one of exp expGauge imp impExp exp_upImp
    exit
fi
