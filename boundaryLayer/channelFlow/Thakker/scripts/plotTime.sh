...
nu=`grep nu $case/constant/transportProperties | tail -1 | awk '{print $NF}' \
    | awk -F ';' '{print $1}'`
delta=1
uTau=`grep uTau $case/log | tail -1 | awk '{print $6}' | awk -F',' '{print $1}'`
Ubar=`grep Ubar $case/log | tail -1 | awk '{print $4}' | awk -F',' '{print $1}'`
Umax=`grep Umax $case/log | tail -1 | awk '{print $9}'`
...
for var in Uf; do
    sed 's%CASETIME%'$case/$time'%g' $case/plotScripts/$var.gmt \
        ...
        | sed 's%UBAR%'$Ubar'%g' | sed 's%UMAX%'$Umax'%g' \
